...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const partner = await prisma.benefitPartner.update({
      where: { id },
      data: parsed.data,
...
/** Apaga o parceiro e (em cascata, ver schema) seus benefícios e resgates. */
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params;
  try {
    await prisma.benefitPartner.delete({ where: { id } });
